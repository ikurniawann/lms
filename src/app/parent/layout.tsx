...
import {
  Users, TrendingUp, Calendar, Wallet, BookOpen,
  Menu, X, Settings, ChevronRight, Bell, FileText, Globe
} from 'lucide-react';
...
export default function ParentLayout({
  ...
  const menuItems = [
    { icon: Users, label: t('parent.menu.dashboard'), href: '/parent/dashboard' },
    { icon: TrendingUp, label: t('parent.menu.academic'), href: '/parent/academic' },
    { icon: Calendar, label: t('parent.menu.attendance'), href: '/parent/attendance' },
    { icon: FileText, label: t('parent.menu.assignments'), href: '/parent/assignments' },
    { icon: BookOpen, label: t('parent.menu.exams'), href: '/parent/exams' },
    { icon: Wallet, label: t('parent.menu.finance'), href: '/parent/finance' },
    { icon: Bell, label: t('parent.menu.announcements'), href: '/parent/announcements' },
    { icon: Users, label: t('parent.menu.messages'), href: '/parent/messages' },
  ];
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-white border-b border-gray-200 z-40 flex items-center justify-between px-4">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-lg font-bold text-gray-900">{t('parent.title')}</span>
        </div>
      ...
      <aside className={`fixed inset-y-0 left-0 z-[70] w-72 bg-white border-r border-gray-200 transform transition-transform duration-300 ease-in-out lg:translate-x-0 overscroll-contain ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="p-4 border-b border-gray-200">
          <div className="flex items-center space-x-3">
            <div className="w-12 h-12 bg-gradient-to-br from-orange-500 to-red-500 rounded-full flex items-center justify-center text-white font-bold text-lg shrink-0">
              BS
            </div>
            <div className="min-w-0">
              <div className="font-semibold text-gray-900 truncate">Budi Santoso</div>
              <div className="text-xs text-gray-500">Orang Tua Siswa</div>
            </div>
          </div>
        </div>
        ...
        <nav className="p-3 space-y-1 overflow-y-auto flex-1" style={{ maxHeight: 'calc(100vh - 200px)' }}>
          {menuItems.map((item) => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`flex items-center justify-between px-4 py-3 rounded-xl transition-all ${isActive ? 'bg-orange-50 text-orange-700 border border-orange-200' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'}`}>
                <div className="flex items-center space-x-3">
                  <Icon className={`w-5 h-5 ${isActive ? 'text-orange-700' : 'text-gray-400'}`} />
                  <span className="font-medium">{item.label}</span>
                </div>
                <ChevronRight className={`w-4 h-4 ${isActive ? 'text-orange-700' : 'text-gray-400'}`} />
              </Link>
            );
          })}
        </nav>
